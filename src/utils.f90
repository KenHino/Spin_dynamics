module utils
    ...
    function eye_cp_sparse(N) result(id)
    ! Generates an indentity matrix of Nension N
        integer, intent(in)      :: N
        type(COO_cdp_type)       :: id

        integer :: i

        call id%malloc(num_rows=N, num_cols=N, nnz=N)
        do i=1,N
            id%index(1,i) = i
            id%index(2,i) = i
            id%data(i) = cmplx(1.0_dp, 0.0_dp, kind=dp)
        end do
    end function eye_cp_sparse
